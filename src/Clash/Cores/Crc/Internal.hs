...
crcEngine
  ...
  -- ^ The CRC
  -> Signal dom Bool
  -- ^ Reset the CRC engine. This has no delay. So this reset can be asserted
...
crcEngineFromParams hwParams@(CrcHardwareParams _ CrcParams{..} _ _) resetCrc inDat = crcOut
    where
...
crcValidatorFromParams
  ...
  -> Signal dom (Maybe (Index nLanes, Vec nLanes (BitVector dataWidth)))
  -> Signal dom Bool
crcValidatorFromParams hwParams@(CrcHardwareParams _ CrcParams{..} _ residues) resetCrc inDat = match
    where
